...
for a in data.get("anime", []):
    if a.get("slug") == "dragon-prince-yuan" or "រាជបុត្រនាគរាជ" in a.get("title", ""):
...
if os.path.exists(db_path):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE anime SET poster_url = ?, banner_url = ? WHERE slug = 'dragon-prince-yuan' OR title LIKE '%រាជបុត្រនាគរាជ%'",
